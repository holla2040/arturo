...
// Sectigo Public Server Authentication Root R46 — signs hollabaugh.com chain
static const char SECTIGO_ROOT_R46_PEM[] =
"-----BEGIN CERTIFICATE-----\n"
...
bool OTAUpdateHandler::startUpdate(const OTARequest& req, const char* currentVersion) {
    ...
    LOG_INFO("OTA", "Update complete, caller will reboot after sending response");
...
bool OTAUpdateHandler::performDownloadAndFlash() {
    ...
    // RAII guard: suspend watchdog for OTA, re-subscribe on any exit.
    // TLS handshake + download can take 30+ seconds, far exceeding
    // the 8s watchdog timeout.
    struct WdtSuspend {
        TaskHandle_t t;
        WdtSuspend() : t(xTaskGetCurrentTaskHandle()) {
            esp_task_wdt_delete(t);
            LOG_INFO("OTA", "Watchdog suspended for OTA download");
        }
        ~WdtSuspend() {
            esp_task_wdt_add(t);
            LOG_INFO("OTA", "Watchdog re-enabled");
        }
    } wdtSuspend;
    ...
    // Download firmware via HTTP (TLS enabled for https:// URLs)
    esp_http_client_config_t httpConfig = {};
    httpConfig.url = _request.firmwareUrl;
    httpConfig.timeout_ms = 30000;
    httpConfig.cert_pem = SECTIGO_ROOT_R46_PEM;
