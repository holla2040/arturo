 ...
 *
 * Ported from pendant2 drivers/gt911.cpp
 */
...
esp_lcd_touch_handle_t touch_gt911_init()
{
    ...
    IO_EXTENSION_Init();
    // Force USB communication mode (prevent CAN mux)
    IO_EXTENSION_Output(IO_EXTENSION_IO_5, 0);
    delay(10);

    pinMode(EXAMPLE_PIN_NUM_TOUCH_INT, OUTPUT);
    IO_EXTENSION_Output(IO_EXTENSION_IO_1, 0);  // Touch reset low

    delay(100);
    digitalWrite(EXAMPLE_PIN_NUM_TOUCH_INT, 0);

    delay(100);
    IO_EXTENSION_Output(IO_EXTENSION_IO_1, 1);  // Touch reset high
